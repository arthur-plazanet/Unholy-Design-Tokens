...
type ThemeCategoryType = (typeof themeCategories)[number];
/**
 ...
 * @returns {boolean}
 */
export function isPartOfTheme(token: DesignToken) {
  return themeCategories.includes(
    token.attributes?.category as ThemeCategoryType
  );
}
